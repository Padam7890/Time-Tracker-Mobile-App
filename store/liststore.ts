    ...
    (set, get) => ({
      ...
      addTask: (task: IListItem) =>
        set((state) => {
          ...
          return {
            tasks: [...updatedTasks, { ...task, id: newId, status: "pending" }],
          };
        }),
      ...
      updateTask: (task: IListItem) =>
        ...
        getTotalMinutes: (taskId: string | number) => {
          ...
          if (task && task.timeLimit) {
            ...
            const [hours, minutes, seconds] = task.timeLimit.split(":").map(Number);
        
            // Validate the split parts to ensure they're numbers
            if (isNaN(hours) || isNaN(minutes) || isNaN(seconds)) {
              console.error(`Invalid timeLimit format for task: ${task.id}, timeLimit: ${task.timeLimit}`);
              return 0;
            }
      ...
      deleteTask: async (id) => {
        ...
        await AsyncStorage.setItem("task-storage", JSON.stringify(get().tasks));
      },
      loadTasks: async () => {
